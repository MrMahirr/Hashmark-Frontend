  ...
  return (
    <section className="bg-hm-surface rounded-card border-[0.5px] border-solid border-hm-border p-5 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          variant="secondary"
          leftIcon={syncMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <GitBranch className="w-4 h-4" />}
          onClick={() => {
            if (!syncMutation.isPending) syncMutation.mutate();
          }}
          disabled={syncMutation.isPending}
        >
          {syncMutation.isPending ? "Syncing..." : "Connect Repo"}
        </Button>
      </div>
      ...
      {isLoading ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-hm-text-muted mt-0.5">
                    Connected on {repo.connectedAt || "recently"}
                  </p>
                </div>
              </div>
              <Button
                ...
                leftIcon={deleteMutation.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Unplug className="w-3.5 h-3.5" />}
                onClick={() => {
        ...
        <div className="text-center py-6">
          <p className="text-sm text-hm-text-secondary">
            No repositories connected yet. Click &quot;Connect Repo&quot; above to sync.
          </p>
        </div>
      )}
    </section>
